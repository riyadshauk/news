...
/**
 * @param article an article (see example below)
 * @example article: {
 *     "source": {
 *         "id": "engadget",
 *         "name": "Engadget"
...
const cleanArticle = article => {
  ...
  return { Title: title, Description: description, URL: url, 'Image URL': urlToImage, 'Publication Time': publishedAt, 'Article Content': content, Source: source.name };
};
...
const populateArticleDetail = article => {
  ...
  <p>
    Published at ${new Date(publishedAt).toLocaleString() || '[NO TIME PROVIDED]'} by <a href=${url}>${(source || {}).name || '[UNKNOWN SOURCE]'}</a>.
  </p>
  <p>
...
export const newsTitlesSection = document.getElementById('national-news-titles');
 ...
 * @param {string} className 
 */
export const displayArticleTitles = (articlesContainer, articles, className = '') => {
  try {
    articlesContainer.innerHTML = '';
    articles.forEach(article => articlesContainer.appendChild(createTitleContainer(article, className)));
